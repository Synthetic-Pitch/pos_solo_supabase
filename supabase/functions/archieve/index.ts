...
Deno.serve(async (req) => {
  ...
  if (
    ...
    return response(req, { message: "Invalid session" }, 401);
  }

  // Keep every authorization condition in the query, exactly like content_verification.
  // This prevents an expired or mismatched session from resolving to a store row.
  ...
  // Scope the update to this store's id only — never a blanket update.
