...
export function isPriceRow(value: unknown): value is PriceRow {
  ...
  return [value.small, value.medium, value.large].every((price) =>
...
function totalAddedCups(entries: unknown): CupTotals {
  ...
  for (const entry of entries) {
    if (!isJsonRecord(entry) || !isNonNegativeSmallInt(entry.quantity)) {
      continue;
    }
    const size = typeof entry.cups_size === "string"
      ? entry.cups_size.trim().toLowerCase()
      : typeof entry.cup_size === "string"
      ? entry.cup_size.trim().toLowerCase()
      : "";
    if (size === "small" || size === "medium" || size === "large") {
      totals[size] += entry.quantity;
    }
  }
  return totals;
}
...
function totalAddedPotatoes(entries: unknown): number {
  ...
  return entries.reduce(
    (total, entry) =>
      isJsonRecord(entry) && typeof entry.kilo === "number" &&
        Number.isFinite(entry.kilo) && entry.kilo >= 0
        ? total + entry.kilo
        : total,
    0,
  );
...
export function buildReceipt(
  ...
  const flavors = new Map<
    string,
    { total_sales: number; revenue: number; payments: PaymentTotals }
  >();
  let totalSales = 0;
  let totalRevenue = 0;
  ...
  for (const sale of sales) {
    ...
    if (
      ...
      !PAYMENT_METHODS.includes(paymentMethod) || !flavor
    ) continue;
    ...
    totalSales += 1;
    totalRevenue += amount;
    const flavorSummary = flavors.get(flavor) ??
      { total_sales: 0, revenue: 0, payments: emptyPaymentTotals() };
  ...
  return {
    inventory_left: {
      small_cups: closing.small,
      medium_cups: closing.medium,
      large_cups: closing.large,
      potatoes: closing.potatoes,
    },
    inventory_movement: {
      small_cups: {
        opening: reconciliation.small_cups,
        added: addedCups.small,
        sold: reconciliation.small_cups + addedCups.small - closing.small,
      },
      medium_cups: {
        opening: reconciliation.medium_cups,
        added: addedCups.medium,
        sold: reconciliation.medium_cups + addedCups.medium - closing.medium,
      },
      large_cups: {
        opening: reconciliation.large_cups,
        added: addedCups.large,
        sold: reconciliation.large_cups + addedCups.large - closing.large,
      },
      potatoes: {
        opening: reconciliation.opening_potatoes,
        added: addedPotatoes,
        sold: reconciliation.opening_potatoes + addedPotatoes -
          closing.potatoes,
      },
    },
    payments,
    flavors: [...flavors.entries()].map(([flavor, summary]) => ({
      flavor,
      ...summary,
    }))
      .sort((a, b) =>
        b.total_sales - a.total_sales || a.flavor.localeCompare(b.flavor)
      ),
